...
const Standings = () => {
  ...
  const calculateStandings = (groupTeamIds) => {
    const standings = groupTeamIds
      .map((teamId) => {
        ...
        const netRunRate =
          played > 0
            ? ((totalRuns - totalRunsConceded) / (totalBalls || 1)).toFixed(2)
            : "0.00";
  ...
  const StandingsTable = ({ standings, groupName, bgColor, borderColor }) => (
    ...
    >
      <div
        className={`px-4 py-3 -mx-6 -mt-6 mb-4 rounded-t-lg text-white font-bold text-center text-xl`}
        style={{
            ...
            <tbody>
              {standings.map((standing, index) => (
                <tr
                  ...
                  style={
                    index < 2
                      ? { borderLeftWidth: "4px", borderLeftColor: "#fbbf24" }
                      : {}
                  }
                >
                  ...
                  <td className="text-center py-3 px-2 font-medium text-gray-600">
                    {standing.draws}
                  </td>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl p-8 text-white">
        ...
        <p className="text-lg opacity-90">Group Stage Points Table</p>
      </div>
      ...
      <div className="card bg-blue-50 border-blue-200">
        ...
        <div className="mt-3 text-sm">
          <div>
            <strong>Pts:</strong> Points (Win = 2, Draw = 1, Loss = 0)
          </div>
          <div className="mt-2">
            <strong>NRR:</strong> Net Run Rate = (Runs Scored - Runs Conceded) /
            Balls Faced
          </div>
        </div>
      </div>
          ...
          <div className="space-y-3 text-sm">
            {groupAStandings.length >= 2 && (
              <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <div className="mt-1">
                  ...
                  {groupAStandings[0]?.team.name} ({groupAStandings[0]?.points}{" "}
                  pts)
                  {groupAStandings[1] && (
                    <span className="ml-4">
                      <span className="font-semibold">2nd: </span>
                      {groupAStandings[1]?.team.name} (
                      {groupAStandings[1]?.points} pts)
                    </span>
                  )}
                </div>
              </div>
            ...
            {groupBStandings.length >= 2 && (
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="mt-1">
                  <span className="font-semibold">1st: </span>
                  {groupBStandings[0]?.team.name} ({groupBStandings[0]?.points}{" "}
                  pts)
                  {groupBStandings[1] && (
                    <span className="ml-4">
                      <span className="font-semibold">2nd: </span>
                      {groupBStandings[1]?.team.name} (
                      {groupBStandings[1]?.points} pts)
                    </span>
                  )}
                </div>
              </div>
      ...
      {groupAStandings.length === 0 && groupBStandings.length === 0 && (
        <div className="card text-center py-12 bg-gradient-to-br from-orange-50 to-red-50 border-orange-300">
          ...
          <p className="text-gray-600 mb-6 max-w-md mx-auto">
            The tournament hasn't been set up yet. Teams need to be assigned to
            groups and matches need to be created.
          </p>
          ...
          <div className="bg-white p-6 rounded-lg border-2 border-orange-300 max-w-2xl mx-auto text-left">
            ...
            <ol className="space-y-3 text-sm text-gray-700">
              <li className="flex gap-3">
                ...
                <span>
                  Go to <strong>Admin Panel</strong> (login if needed)
                </span>
              </li>
              <li className="flex gap-3">
                <span className="font-bold text-orange-600 flex-shrink-0">
                ...
                <span>
                  Click the <strong>"Matches"</strong> tab
                </span>
              </li>
              <li className="flex gap-3">
                <span className="font-bold text-orange-600 flex-shrink-0">
                ...
                <span>
                  Click the <strong>"🏆 Start Tournament"</strong> button at the
                  top
                </span>
              </li>
              <li className="flex gap-3">
                ...
                <span>
                  The system will automatically assign teams to Group A & B and
                  create all 12 group matches
                </span>
              </li>
              <li className="flex gap-3">
                <span className="font-bold text-orange-600 flex-shrink-0">
                ...
                <span>
                  Return to this page to see the standings (they'll update as
